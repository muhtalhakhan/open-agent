...
export interface ToolDefinition<Args extends Record<string, unknown> = Record<string, unknown>> {
   ...
   * control — a fetched page, an API response, search results. The agent loop
   * fences that output as data before the model sees it, and a task that has
   * read any stops being covered by remembered approvals (see
   * `ToolRegistry`), since the next call might be the content's idea rather
   * than the user's.
   */
