...
- **`SupermemoryProvider`** (`src/supermemory-provider.ts`) — the reference implementation, backed by [Supermemory](https://github.com/supermemoryai/supermemory): automatic fact extraction, contradiction/temporal handling, and auto-maintained user profiles, on top of hybrid RAG+memory search. Works unchanged against the hosted API or a self-hosted `supermemory-server` — only the client's `baseURL` differs.
