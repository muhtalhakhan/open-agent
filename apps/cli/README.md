...
- **Filesystem tools**: `read_file`, `list_directory`, `search_files` and `write_file`, mounted when `FILES_TOOL=1` and all confined to `FILES_ROOT` (the launch directory unless set). Reading, listing and searching are `safe`; every write prompts. See `packages/tools-files/README.md`.
- **Shell tool**: `run_command`, mounted when `SHELL_TOOL=1`, running under `SHELL_ROOT` (falling back to `FILES_ROOT`, then the launch directory). `ask`-level, so every command prompts. See `packages/tools-terminal/README.md`.
