...
- **Plan mode** (`--plan`): file edits are reviewed as a diff before they run. When the model proposes a `write_file`, the CLI renders the exact change (`--- a/…` / `+++ b/…`, colored when writing to a real terminal) and asks for once / for-the-task / no — the approval _is_ the diff review. Non-file tools prompt exactly as they do without `--plan`, so shell/HTTP approvals are unchanged. Interactive-only, since a CI job has no one to review the diff; there is no "always" scope, because a write that never prompts again is what plan mode exists to prevent.
