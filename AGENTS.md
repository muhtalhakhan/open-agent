...
`packages/tools` is a README-only placeholder; the filesystem tools live in
`packages/tools-files`. `packages/security` holds the secret stores (OS keychain). The permission and approval system lives in
